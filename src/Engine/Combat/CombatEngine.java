...
public class CombatEngine
{
  ...
  public static StrikeParams calculateTerrainDamage( Unit attacker, Path path, Location target, GameMap map )
  {
    ...
    return new StrikeParams(
        ...
        weapon.getDamage(target), map.getEnvironment(path.getEndCoord()).terrainType.getDefLevel(),
        false, true); // Terrain doesn't take luck damage
  }
                                                     ...
                                                     boolean isSim)
  {
    ...
    // Set up our scenario.
    BattleParams attackInstance = StrikeParams.getAttack(context, isSim);
    ...
    if( context.canCounter && (context.defender.getPreciseHP() > defenderHPLoss) )
    {
      ...
      BattleParams defendInstance = StrikeParams.getCounterAttack(context, counterHP, isSim);
